...
const GetBooking = (props: IGetBooking) => {
    ...
    useEffect(() => {
        if (futureModal) {
            axiosPrivate.get(`/client/booking/future/${props.accepted}`)
                .then(res => {
                    if (res.data !== null) {
            ...
            axiosPrivate.get(`/client/booking/past/${props.accepted}`)
                .then(res => {
                    if (res.data !== null) {
                        setBookings(res.data.client.bookings);
                    } else {
            ...
            axiosPrivate.get(`/client/booking/present/${props.accepted}`)
                .then(res => {
                    if (res.data !== null) {
                        setBookings(res.data.client.bookings);
                    } else {
        ...
        <Grid container columnSpacing={2} rowSpacing={3} mb={3} mt={2} maxWidth="0.9" justifyContent="center" mx="auto">
            ...
            <Grid item xs={12} textAlign="center">
                <ButtonGroup aria-label="button group">
                    <Button id="past" onClick={handleModals} className={pastModal ? "selected" : "notSelected"} >Passées</Button>
                    <Button id="present" onClick={handleModals} className={presentModal ? "selected" : "notSelected"}>Du jour</Button>
                    <Button id="future" onClick={handleModals} className={futureModal ? "selected" : "notSelected"}>A venir</Button>
                </ButtonGroup>
            </Grid>
            {
