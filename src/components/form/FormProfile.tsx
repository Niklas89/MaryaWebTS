...
const FormProfile = () => {
  ...
  const validationShema = Yup.object().shape({
    lastName: Yup.string()
      ...
      .required("Merci de remplir le champ nom"),
    firstName: Yup.string()
      .matches(/^[A-Za-z]+$/, "Le nom doit contenir que des lettres.")
      .required("Merci de remplir le champ prenom"),
    email: Yup.string()
      .email("Votre e-mail n'est pas valide")
      .required("Merci de remplir le champ e-mail"),
    address: Yup.string().required("Merci de remplir le champ adresse"),
    city: Yup.string()
      .matches(/^[A-Za-z]+$/, "La ville doit contenir que des lettres.")
      .required("Merci de remplir le champ ville"),
  ...
  const handleSubmit = useCallback(
    (values: FormikValues, callback: any) => {
      ...
      patchQuery("/client/edit/", postData)
  ...
  return (
    <>
      ...
      {renderForm}
      <Grid container mb={5} direction="row" justifyContent="center">
        <Grid item xs={0} mt={2}>
          <Button
            variant="contained"
            component={Link}
